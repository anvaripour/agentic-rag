...
class LocalDataAgent:
    def __init__(self, ltm: LongTermMemory):
        self.ltm = ltm
        self.mcp_client = (
            MCPClient([sys.executable, "-m", "agentic_rag.mcp.servers.local_pdf_server"])
    ...
    def retrieve(self, query: str, k: int = 5) -> List[Tuple[str, float]]:
        if self.mcp_client:
            try:
                ...
                # fallback to local FAISS if MCP server errors
                print(f"[WARN] MCP client failed, falling back to FAISS: {e}")
